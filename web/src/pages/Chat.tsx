...
const Chat = () => {
    ...
    return (
        <section className="w-full h-[100dvh] flex flex-row justify-start items-start bg-background text-foreground overflow-hidden">
            ...
            {/* Sidebar */}
            <div className={cn(
                "fixed md:relative z-20 md:z-auto h-full w-72 md:w-64 flex flex-col bg-background border-r border-border transition-transform duration-300 shadow-xl md:shadow-none",
                ...
                <div className="p-3 flex items-center gap-2 border-b">
                    ...
                    <Button variant="ghost" size="icon" className="md:hidden shrink-0 border" onClick={() => setSidebarOpen(false)}>
            ...
            <div className="flex-1 flex flex-col h-full overflow-hidden bg-background min-w-0">
                {/* Header */}
                <div className="flex items-center px-3 py-2 border-b border-border bg-background/80 backdrop-blur z-10 shrink-0 gap-2">
                    <Button variant="ghost" size="icon" className="md:hidden shrink-0" onClick={() => setSidebarOpen(true)}>
                        <PanelLeftIcon className="w-5 h-5" />
                    </Button>
                    <h1 className="text-base font-medium opacity-80 flex-1 truncate">AI Chat</h1>
                ...
                <div className="flex-1 overflow-y-auto p-3 sm:p-4 space-y-4">
                    ...
                    {messages.map((m) => (
                        <div key={m.id} className={cn("flex w-full", m.role === "user" ? "justify-end" : "justify-start")}>
                            <div className={cn("max-w-[90%] sm:max-w-[85%] rounded-2xl p-3 sm:p-4 shadow-sm",
                    ...
                    {(streamedResponse || activeTool) && (
                        <div className="flex w-full justify-start">
                            <div className="max-w-[90%] sm:max-w-[85%] rounded-2xl p-3 sm:p-4 shadow-sm bg-card border border-border text-card-foreground flex flex-col gap-2">
                                {activeTool && (
