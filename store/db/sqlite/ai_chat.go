...
// EnsureAIChatTables creates the AI chat tables if they don't exist.
// Called once at startup via the Store.Migrate hook.
...
func (d *DB) UpdateAIChatSession(ctx context.Context, update *store.UpdateAIChatSession) (*store.AIChatSession, error) {
	...
	stmt := fmt.Sprintf(
		`UPDATE ai_chat_session SET %s WHERE uid = ?
